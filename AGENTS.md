...
- `bazel test //internal/controller/... --test_env "KUBEBUILDER_ASSETS=$(bazel --bazelrc=.github/workflows/ci.bazelrc run --run_in_cwd @io_k8s_sigs_controller_runtime_tools_setup_envtest//:setup-envtest -- use -p path)"` - Run controller tests with `envtest` binaries.
